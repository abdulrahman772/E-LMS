...
if ($do == 'search') {
  ...
  <div class="row">
      <form class="text-center" style=" margin:0; padding:50px 0 0 0; width:100%;" action="search.php?do=search" method="POST" enctype="multipart/form-data">
          <div class="col-sm-12 col-sm-offset-3 col-md-6 col-md-offset-3" style="">
              <div class="form-group" style="">
                Employee ID OR Name  <input class="form-control" type="search" placeholder="Employee ID OR Name"  name="search" required >
              </div>
          </div>
...
if ($stmt->rowCount() > 0) {
   $Emp_id =$row['Emp_id'];
      $sstatus = 1 ;
      $stmt = $con->prepare("SELECT * FROM transactions WHERE  hod_status = ? AND  Dept_id =? AND  Emp_id =?  LIMIT 9 ");
      $stmt->execute(array($sstatus,$de,$Emp_id));
      $row = $stmt->fetchAll();
        $count = $stmt->rowCount();
      if ($stmt->rowCount() > 0) {
        ?>
        <div class="">
        <header class="" style="margin-top:0; padding: 5px;">
            <p class="text-center" style="padding-top:0;color:#00669b;font-weight:bold;font-size:25px;">Employee Leave Transactions</p>
        </header>
        <div class="">
    ...
    $stmt = $con->prepare("SELECT * FROM transactions WHERE  hod_status = ? AND  Dept_id =?  AND  Emp_id =? LIMIT 9 ");
      ...
      if ($stmt->rowCount() > 0) {
        ...
        <div class="">
        <header class="" style="margin-top:0; padding: 5px;">
            <p class="text-center" style="padding-top:0;color:#00669b;font-weight:bold;font-size:25px;">Employee Leave Transactions reject</p>
        </header>
        <div class="">
            <div class="table" align="center">
              <br>
