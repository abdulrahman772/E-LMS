...
//**************************************************************************************************
if ($do == 'manage') {
  $stmt = $con->prepare("SELECT * FROM transactions WHERE hod_status = 1 ");
  $stmt->execute();
  $row = $stmt->fetchAll();
  $count = $stmt->rowCount();
  if ($count > 0) {
      ...
      <div class="" style="">
          <header class="" style="margin-top:0;padding: 5px;">
              <p class="w3-center" style="padding-top:0;color:#00669b;font-weight:bold;font-size:25px;">accepted =
                <?php
                $stmt = $con->prepare("SELECT COUNT(trans_id) FROM transactions WHERE hod_status = 1");
                $stmt->execute();
                      ?>
...
<?php
  ...
  echo "<div class='alert alert-success'>   erorr </div>";
  echo "</div>";
}
}
//*************************************************************************************************8
